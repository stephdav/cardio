...
public class StoryServiceImpl implements StoryService {
	...
	private Story update(Story story, boolean updateTimestamp) throws CardioTechnicalException, CardioFunctionalException {
		checkStoryProperties(story);
	...
	public String export() throws CardioTechnicalException {
		...
		for (Story us : stories) {
			if (us.getAssignedUser() == null) {
				sb.append(String.format(SQL_EXPORT_WITHOUT_USER, escapeSpecialCharacters(us.getDescription()), us.getStatus().toString(),
						convertLocalDateTime(us.getLastUpdate()), us.getContribution(), us.getEstimate()));
			} else {
				sb.append(String.format(SQL_EXPORT, escapeSpecialCharacters(us.getDescription()), us.getStatus().toString(),
						convertLocalDateTime(us.getLastUpdate()), us.getContribution(), us.getEstimate(), us.getAssignedUser()));
			}
		}
